...
let AddIntervalWindow = React.createClass({
  propTypes: {
    onSearchDataButtonClickFn: React.PropTypes.func, // enter from Alarm
    onsetShowAddIntervalDialog: React.PropTypes.func,
  },
  _onAction(action) {
    let analysisPanel = this.props.analysisPanel;
    if (action === 'draw') {
      MultiTimespanAction.convert2Stable();
      if (!!this.props.onSearchDataButtonClickFn) {
        this.props.onSearchDataButtonClickFn(true);
      } else {
