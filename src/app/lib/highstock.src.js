	...
	colorizeItem: function (item, visible) {
		...
		if (legendLine) {
            ...
            if (item.legendSASymbol) {
                item.legendSASymbol.attr({ fill: symbolColor });
                item.legendSBSymbol.attr({ fill: symbolColor });
            }

            if (item.legendLineSymbol) {
                item.legendLineSymbol.attr({ fill: symbolColor });
            }
            if (item.legendColumnSymbol) {
                item.legendColumnSymbol.attr({ fill: symbolColor });
            }
            //REM add end
		if (legendSymbol) {
	...
	destroyItem: function (item) {
		...
		// destroy SVG elements
            each(['legendItem', 'legendLine', 'legendSymbol', 'legendGroup',
                /*add by rem*/'deleteButton', 'deleteButtonCt', 'legendSASymbol', 'legendSBSymbol', 'legendLineSymbol', 'legendColumnSymbol'/*add by rem*/], function (key) {
			if (item[key]) {
...
//add by rem -- start --
                ...
                if (item.options.enableDelete) {
                    ...
                    item.deleteButton = renderer.path([
                    ...
                    }).css({ 'z-index': 10 }).on('mouseover', function () {
                        ...
                        if (clickchart) {
                            fireEvent(clickchart, 'deleteButtonClick', { uid: item.options ? item.options.uid : item.uid });
		...
		itemWidth = item.checkboxOffset =
			...
			symbolWidth + symbolPadding + bBox.width + itemDistance + (showCheckbox ? 20 : 0)/*add by rem */ + (allowDelete ? 20 : 0)/*add by rem */;
	...
	render: function () {
		...
		legend.renderTitle();
            // REM add start
            if (!legend.tooltip) {
                legend.tooltip = new Tooltip(chart, extend(chart.options.tooltip, { animation: false }));
            }
            //REM add end
	...
	getSymbol: function () {
        ...
        */
        drawAllLegendSymbols: function (legend, item, legendSwitchList) {
            var series = this, nextType = 'line',
                lineProto = Highcharts.seriesTypes.line.prototype,
                renderer = this.chart.renderer,
                isStacking = (series.type === 'column' && series.options.stacking !== null),
                tipStrMap = { 'line': I18N.EM.Legend.ToLine, 'column': I18N.EM.Legend.ToColumn, 'stacking': I18N.EM.Legend.ToStacking },
            ...
            this.symbolGroup = item.symbolGroup = renderer.g()
                   ...
                   .on('mouseout', function (evt) {
                       ...
                       if (legend.tooltip) { legend.tooltip.label.fadeOut(); }
                       if (series.type == 'line') {
                       ...
                       var isStacking = series.options.stacking !== null,
                           nextType = series.getNextSwitchType(isStacking ? 'stacking' : series.type, legendSwitchList);
                       if (legend.tooltip) { legend.tooltip.label.fadeOut(); }
                       ...
                       fireEvent(series.chart, 'switchChartTypeButtonClick', { chartType: nextType, seriesItem: series });
                   });
        ...
        updateSieriesYaxis: function (type) {
            ...
            if (type == 'stacking') {
                ...
                for (i = 0; i < len; i++) {
                    ...
                    if (series.type == 'column' && series.options.stacking !== null && me.options.option.commodityId == series.options.option.commodityId) {
        ...
        showSpecificSymbol: function (type, series) {
            ...
            if (type == 'line') {
                ...
                ls.attr({ visibility: 'visible' });
                cs && cs.attr({ visibility: 'hidden' });
                sa && sa.attr({ visibility: 'hidden' });
                sb && sb.attr({ visibility: 'hidden' });
            } else if (type == 'column') {
                cs.attr({ visibility: 'visible' });
                l && l.attr({ visibility: 'hidden' });
                ls && ls.attr({ visibility: 'hidden' });
                sa && sa.attr({ visibility: 'hidden' });
                sb && sb.attr({ visibility: 'hidden' });
            } else {
                sa.attr({ visibility: 'visible' });
                sb.attr({ visibility: 'visible' });
                l && l.attr({ visibility: 'hidden' });
                ls && ls.attr({ visibility: 'hidden' });
                cs && cs.attr({ visibility: 'hidden' });
            }
        ...
        drawLineLegendSymbol: function (legend) {
            var options = this.options,
               markerOptions = options.marker || { radius: 3 },
               radius,
...
extend(Legend.prototype, {
	...
	(useHTML ? legendItem : item.legendGroup).on('mouseover', function () {
		...
		.on('click', function (event) {
		    ...
		    //rem add start
			if (item) {
			    item.chart.trigger(strLegendItemClick, { event: event, seriesItem: item,
...
extend(Point.prototype, {
	...
	onMouseOver: function (e) {
		...
		// update the tooltip
		if (tooltip && (!tooltip.shared || series.noSharedTooltip) /*rem add start*/ && !series.options.notShowTooltip/*rem add end*/) {
