...
let CalendarManager = {
  ...
  showCalendar: function (cmp, type) {
    ...
    if (cmp && cmp.showCalendar) {
      if (!this.data) {
        window.REM.popupNotes(I18N.EM.CannotShowCalendarByTimeRange);
        return;
      }
  ...
  getTimeRange: function (step, type, range) {
      ...
      if (type == 'hotColdSeason') {
          ...
          switch (step) {
              case 0://raw time
              case 1://hour
                  ret = range.filter(item=>{return item.CalendarType == 3;});
                  break;
              case 2://day
                  ret = range.filter(item=>{return item.CalendarType == 1;});
                  break;
              case 5://week
              case 3://month
              case 4://year
  ...
  convertData: function (range) {
      var color = { 1: '#eaeaea', 3: '#eaeaea', 4: '#fcf0e4', 5: '#e3f0ff' };
      var text = { 1: I18N.Setting.Calendar.Holiday, 3: I18N.Common.Button.Calendar.ShowHoliday, 4: I18N.Setting.Calendar.WarmSeason, 5: I18N.Setting.Calendar.ColdSeason };
      var cal = range;
      var j2d = CommonFuns.DataConverter.JsonToDateTime;
      var ret = [];
